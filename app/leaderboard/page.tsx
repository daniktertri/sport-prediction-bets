      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="sm:hidden space-y-3">
          {users.map((user, index) => {
            ...
            return (
              <Card key={user.id} className={isCurrentUser ? 'border-accent' : ''}>
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <div className={`text-lg font-bold ${isCurrentUser ? 'text-accent' : 'text-text-primary'}`}>
                      {user.totalPoints}
                    </div>
            ...
            <table className="w-full">
              <thead className="bg-bg-tertiary">
                <tr>
                  ...
                  <th className="px-4 lg:px-6 py-3 text-center text-xs font-semibold text-text-secondary uppercase tracking-wider">
                    Exact
                  </th>
                  <th className="px-4 lg:px-6 py-3 text-center text-xs font-semibold text-text-secondary uppercase tracking-wider">
                    Winner
              ...
              <tbody className="bg-bg-secondary divide-y divide-bg-tertiary">
                {users.map((user, index) => {
                  ...
                  return (
                    <tr
                      key={user.id}
                      className={isCurrentUser ? 'bg-accent/10 hover:bg-accent/20' : 'hover:bg-bg-tertiary/50'}
                    >
        ...
        <Card className="mt-6 sm:mt-8">
          ...
          <ul className="space-y-2 text-xs sm:text-sm text-text-secondary">
            <li className="flex items-center gap-2">
              <span>✅</span>
              <span>Exact score: <span className="font-semibold text-text-primary">+10 pts</span></span>
            </li>
            <li className="flex items-center gap-2">
              <span>✅</span>
              <span>Winner only: <span className="font-semibold text-text-primary">+3 pts</span></span>
            </li>
            <li className="flex items-center gap-2">
              <span>✅</span>
              <span>Man of match: <span className="font-semibold text-text-primary">+3 pts</span> (bonus)</span>
            </li>
            <li className="flex items-center gap-2">
              <span>❌</span>
